...
```{r plot-settings}
plot_title <- "35 year old female who breastfed two children"
out_name <- "35yo_Female_BF"

drop_vars <-  c("conc_t1", "MilkClearance", "PlacXfer", "MatXfer")
vars_order = c("T[1/2] (years)", "V[d] (L/kg)","DWI (μg/L)","C[bgd] (μg/L)", "BW (kg)", 
                        "Percent Tap (%)", "MatRemove", "DWC (μg/L)")
```
...
param_list_PFOA_ucmr5 <-  param_make(chemical = "PFOA", 
                                    ...
                                    perc_tap = c(.9), 
                                    gender = "Female", 
                                    BW = 71.45, niter = 1, timestep = 30, 
                                    sd = as.Date("1987-01-1"), # start date of simulation, 
                                                               # simulated individuals birthdate  (YYYY-MM-DD)
                                    ed = as.Date("2022-01-01"),
                                    numChild = 2, # number of user's children
                                    child_gender = c("Female", "Male"), # children's genders
                                    child_bday = c("2019-06-01", "2017-10-01"), # children's birthdays
                                    child_bf_mo = c(6,6)) # number of months child was breastfed. 
```
...
Customized tornado plot call with input together = "DWC (μg/L)". This input pastes the labels together in one string to avoid over plotting. The input h_just = 0 shifts the bar labels to the right to avoid over plotting.
...
# plot
g <- ggtornado(data = dframe, baseline = 2.0,
          var = "var", level = "level", value = "value", result = "pred_serum_ug_L", 
          Bkgd =  1.262484175, h_just = 0, together = "DWC (μg/L)",
...
Customized tornado plot call with input together =  c("MatRemove", "DWC (μg/L)" ). This input pastes the labels together in one string to avoid over plotting. The input h_just = 0 shifts the bar labels to the right to avoid over plotting.
...
g <- ggtornado(data = dframe, baseline = 5.5,
          var = "var", level = "level", value = "value", result = "pred_serum_ug_L",
          Bkgd = 3.414556018, h_just = 0, together = c("MatRemove", "DWC (μg/L)" ),
          var_order = vars_order)
...
pfhxs <- mc_create_F( plist = param_list_PFHxS_ucmr5,  
                      sourceData = sourceData , pfas = param_list_PFHxS_ucmr5$chemical,
                      HL_range =  c(2.4, 8.3, 13.5) , 
                      V_d_range =  c(0.15, 0.29, 0.45),
                      Bkgd_range = c(0.3, 0.805082068, 2.14 ),
                      MatRemove_range = c(0.377,0.39,0.4))
...
g <- ggtornado(data = dframe, baseline = 6.9,
          var = "var", level = "level", value = "value", result = "pred_serum_ug_L" ,
          Bkgd = 0.805082068, var_order = vars_order)
...
pfna <- mc_create_F( plist = param_list_PFNA_ucmr5,  
                      sourceData = sourceData , pfas = param_list_PFNA_ucmr5$chemical, 
                      HL_range =  c(1.9, 2.35, 9.5) , 
                      V_d_range =  c(0.1, 0.19, 0.3),
                      Bkgd_range = c(0.17,	0.383562351,	0.86 ),
                      MatRemove_range = c(0.28,	0.29,	0.299))
...
g <-  ggtornado(data = dframe, baseline = 3.6,
          var = "var", level = "level", value = "value", result = "pred_serum_ug_L",
          Bkgd = 0.383562351,
          var_order = vars_order)
