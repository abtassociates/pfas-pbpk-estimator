---
title: "PBPK Model of PFAS Chemicals in Blood Serum - Breast Feeding 6 month old female infant"
...
```{r plot-settings}
plot_title <- "6 month old female"
out_name <- "6mo_Female"

drop_vars <- c("conc_t1", "DWI", "MatRemove")
vars_order = c("V[d] (L/kg)",  "T[1/2] (years)", "DWC (μg/L)", "BW (kg)",
                        "LTF",  "C[bgd] (μg/L)", "Milk Clearance",
                        "PTF", "Percent Tap (%)")
```
...
param_list_PFOA_ucmr5 <-   param_make(chemical = "PFOA", 
                                    ...
                                    perc_tap = c(.9), 
                                    gender = "Female", 
                                    BW = 7.45, niter = 1, timestep = 30, # BW = p50 @ age 6 months in source file
                                    sd =  as.Date("2021-06-1"), # start date of simulation, 
                                                                # simulated individuals birthdate (YYYY-MM-DD)
                                    ed = as.Date("2022-01-01"),
                                    fed_most = "breast milk",
                                    fed_mo = 6)
```
...
Customized tornado plot call with input  together = c("Percent Tap (%)"). This input pastes the labels together in one string to avoid over plotting. 

```{r PFOA}


pfoa <- mc_create_child( plist = param_list_PFOA_ucmr5, Sex = param_list_PFOA_ucmr5$gender, 
...
# plot
g <- ggtornado(data = dframe, baseline = 3.229338,
          var = "var", level = "level", value = "value", result = "pred_serum_ug_L",
          Bkgd = 1.89681023, together = c("Percent Tap (%)"),
...
Customized tornado plot call with line_label = FALSE. This input controls whether the NHANES Mean Background Serum label is printed.
...
pfos <- mc_create_child( plist = param_list_PFOS_ucmr5, Sex = param_list_PFOS_ucmr5$gender,
                      sourceData = sourceData , pfas = param_list_PFOS_ucmr5$chemical,
...
# 3.905498 and 3.905499 appear in baseline, so we round to 3.90550
g <- ggtornado(data = dframe, baseline = 3.9055,
          var = "var", level = "level", value = "value", result = "pred_serum_ug_L",
          Bkgd = 3.699068318, line_label = F,
          var_order = vars_order)
...
Customized tornado plot call with input together = c("C[bgd] (μg/L)", "Milk Clearance", "Percent Tap (%)"). This input pastes the labels together in one string to avoid over plotting. 
...
pfhxs <- mc_create_child( plist = param_list_PFHxS_ucmr5, Sex = param_list_PFHxS_ucmr5$gender, 
                      ...
                      Bkgd_range = c(0.25, 0.756904737, 2.24 ),
                      MatXfer_range = c(0.0135,0.014,.033), 
                      MilkClear_range = c(0.11, 0.127,.14), 
                      PlacXfer_range = c(0.67, 0.7, 0.72))
...
# both 3.231228 & 3.231229 appear in the baseline runs, so we round to 3.23123
g <- ggtornado(data = dframe, baseline = 3.342806,
          var = "var", level = "level", value = "value", result = "pred_serum_ug_L", 
          Bkgd = 0.756904737, together = c("C[bgd] (μg/L)", "Milk Clearance", "Percent Tap (%)"),
          var_order = vars_order)
...
Customized tornado plot call with input together = c("PTF", "C[bgd] (μg/L)", "Percent Tap (%)", "BW (kg)", "Milk Clearance"). This input pastes the labels together in one string to avoid over plotting. The input h_just = 0 shifts the bar labels to the right to avoid over plotting.
...
pfna <- mc_create_child( plist = param_list_PFNA_ucmr5, Sex = param_list_PFNA_ucmr5$gender, 
                      sourceData = sourceData , pfas = param_list_PFNA_ucmr5$chemical,
                      HL_range =  c(1.9, 2.35, 9.5) , 
                      V_d_range =  c(0.1, 0.19, 0.3),
                      Bkgd_range = c(0.28, 0.758818721, 2.07 ),
                      MatXfer_range = c(.009,.01,.14), 
                      MilkClear_range = c(0.074, 0.0903,.11), 
                      PlacXfer_range = c(0.512, 0.53, 0.94))
...
# 1.3813677 and 1.3813678 appear in baseline, so we round to 1.381368
g <- ggtornado(data = dframe, baseline = 1.381368,
          var = "var", level = "level", value = "value", result = "pred_serum_ug_L",
          Bkgd = 0.758818721, h_just = 0, together = c("PTF", "C[bgd] (μg/L)", "Percent Tap (%)", 
                                                       "BW (kg)", "Milk Clearance"),
          var_order = vars_order)
