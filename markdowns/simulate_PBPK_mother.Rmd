---
title: "PBPK Model of PFAS Chemicals in Blood Serum - Mother"
...
The purpose of this model is to calculate the amount of PFAS, PFOA, PFHxS or PFNA (ug/L) in an individual's blood serum, given characteristics of the individual, their tap water concentration & consumption. It performs a Monte Carlo simulation to introduce variation into 14 different variables that define an individual for simulation. This specific example shows PFOA estimation for an adult female who has given birth to three children, two of which were breastfed. 
...
param_list <- list(chemical = "PFOA", 
                   ...
                   water_concs_dates = c("1970-1-1", "2010-1-1"), 
                   perc_tap = c(.9,.75), # percentage
                   perc_tap_dates = c("1900-1-1", "2000-1-1"),
                   ...
                   numChild = 3, 
                   child_gender = c("Female", "Male", "Male"), 
                   child_bday = c( "2003-10-25", "2010-1-10", "2010-12-4"), 
                   child_bf_mo = c(0, 6, 6), 
                   
                   creation_date = as.Date("2022-12-31"), # date output created or body weight provided 
                   # we set a creation_date for reproducibility
                   # it also locks the weight provided to a women aged 42
                   niter = 1000, 
                   timestep = 30, 
                   sd = as.Date("1980-10-1"), 
...
-------|------------|---------------------------------------------
 ...
 Serum_a | Serum concentration after elimination | Serum_b * DecayPerStep. While women are breastfeeding, we substitute CRBFPerStep for DecayPerStep to add additional decay for breast milk clearance. These are calculated with MC varied HL, MC varied MilkClear and MC varied MatRemove
...
# without the argument full = TRUE, only the last timestep will be returned. This is used in keeping object sizes small in model validation when running many individuals. To plot over time, we must have all timesteps.
toc()
```
...
save(results, file = here("output", "PBPK", Sys.Date(), "PBPK_Mother.Rdata"))
load(file = here("output", "PBPK", Sys.Date(), "PBPK_Mother.Rdata"))
