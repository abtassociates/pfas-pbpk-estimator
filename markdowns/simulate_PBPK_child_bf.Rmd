---
title: "PBPK Model of PFAS Chemicals in Blood Serum - Breastfed Child"
...
The purpose of this model is to calculate the amount of PFAS, PFOA, PFHxS, or PFNA (ug/L) in an individual's blood serum, given characteristics of the individual, their tap water concentration & consumption. It performs a Monte Carlo simulation to introduce variation into 14 different variables that define an individual for simulation. This specific example shows PFOA estimation for a child who was primarily breastfed. 
...
This section of code allows users to change settings of the model. The following table defines the parameters

Variable | Description
---------|----------------------
chemical | "PFOA", "PFOS", "PFNA", or "PFHxS"
water_concs | water concentrations (ug/L) for each date
water_concs_dates | dates to apply changes to water concentration   
perc_tap | percent of daily water intake that comes from tap water as a fraction (e.g. 90% - .9)
perc_tap_dates | dates to apply changes to perc_tap
gender | "Female" or "Male"
BW | body weight (kg)
fed_most | "breast milk", "tap water", or "other formula", represents whether the user was mostly breast fed, fed formula made with tap water, or other store bought formula. 
fed_mo | number of months after birth they were fed mostly fed_most
mom_pct_tap | the mother's percent of water intake from tap water during pregnancy and breastfeeding
numChild | number of children 
child_gender | "Female" or "Male", represents each child's gender
child_bday | each child's birthday
child_bf | TRUE or FALSE, represents whether each child was breastfed.
creation_date | manual override for matching calculations run on previous days, defaults to today's date
niter | number of Monte Carlo iterations 
timestep | number of days between calculations
sd | start date, the individual's birthday
ed | end date, the last date to simulate. May not be exact due to timestep

```{r set-param}
# the user sets 
param_list <- list(chemical = "PFOA", # PFAS chemical
                   water_concs = c(.25,.02), # concs in tap water (ug/L)
                   water_concs_dates = c("1990-1-1", "2019-1-1"), 
                   # dates water_concs sampled
                   
                   perc_tap = c(.9,.25), 
                   # percent drinking water intake from tap water 
                   perc_tap_dates = c("1900-1-1", "2021-1-1"), 
                   # initialized in 1900, date the perc_tap changed
                   
                   # mother's percent drinking water intake from tap water 
                   # during pregnancy and breastfeeding. If NA, assume 100%
                   mom_pct_tap = 1,
                   
                   gender = "Female", # user gender
                   BW = 30, # user current body weight (kg) 
                   
                   # If you are a child: (ignored if over 6 years old)
                   
                   fed_most = "breast milk", # user was fed mostly "breast milk", 
                   # "tap water", or "other formula" when they were an infant. 
                   fed_mo = 6, # for how many months? (ignored when fed_most = "tap water")
                   # default 6 months for breastfeeding (max value)
                   # default 12 months for other formula (not a maximum)
                   
                   # If you are a mother: (ignored if gender = Male or numChild = 0)
                   numChild = 0, # number of user's children
                   child_gender = c(NA), # children's genders
                   child_bday = c(NA), # children's birthdays
                   child_bf_mo = c(NA), # number of months child was breastfed. 
                   
                   creation_date = Sys.Date(), # date output created
                   niter = 10, # number of Monte Carlo iterations for simulation. Recommendation: set to 10 for initial testing, and to 1000 for full simulation. Values over 1000 not recommended to avoid intensive computing.  
                   timestep = 30, # Length of timestep (days between calculations). 30 day timestep is recommended for ease of simulation. Shorter timesteps are slower to run. 
                   sd = as.Date("2017-5-1"), # start date of simulation, user's birthdate  (yyyy-m-d)
                   ed = as.Date("2022-12-31")) # end date of simulation
```
...
This section of code creates a dataframe of Monte Carlo variables that will be mapped as inputs into the master function. The following table describes their distribution assumptions
...
The master function constructs a dataframe for a single iteration of MC variables, populates it and performs the serum calculations. The output of a single iteration is a list including the entire dataframe as a result and a restatement of the parameters
...
res_sum$Median <- rowMedians(res_df)# Row Means functions were buggy
...
save(results, file = here("output", "PBPK", Sys.Date(), "PBPK_Child_BF.Rdata"))
load(file = here("output", "PBPK", Sys.Date(), "PBPK_Child_BF.Rdata"))
